    ...
    def daily_graph_generator(self, start_date):
        ...
        if not os.path.exists(filename):
            ...
            for stock in G.nodes:
                ...
                start_idx = self.data[stock].index.get_loc(start_date)
                G.nodes[stock]['history'] = self.data[stock]['Adj Close'].iloc[max(0, start_idx - 30):start_idx + 1].tolist() # Store closing values for last 30 days
                G.nodes[stock]['target'] = self.data[stock]['Adj Close'].iloc[start_idx ] # Store the closing value for the next day        
